...
function FormOrder() {
  ...
  const usedTableNumbers = useMemo(() => {
    return orders
      .filter((order) => order.tableNumber && !order.takeaway && order.status != 'Selesai')
      .map((order) => String(order.tableNumber));
  }, [orders]);

  const availableTableNumbers = useMemo(() => {
    return tableNumber.filter((t) => !usedTableNumbers.includes(t.name));
  }, [usedTableNumbers]);
    ...
    <div className="flex flex-col gap-6">
      ...
      {!takeaway && availableTableNumbers.length !== 0 && (
        <SelectField
          ...
          label="Nomor Meja"
          value={table}
          options={availableTableNumbers.length > 0 ? availableTableNumbers : []}
          onChange={setTable}
          placeholder="Pilih nomor meja"
        />
      )}
      {availableTableNumbers.length == 0 && <p className="text-red-400">Tidak ada meja tersedia</p>}
