...
import { useCreateOrderMutation, useGetOrdersQuery } from '@/lib/redux/api/orderApi';
...
function PaymentModal({ name, table, takeaway }) {
  ...
  const ListCard = ({ product }) => {
    return (
      <div className="grid grid-cols-3 text-sm sm:text-base py-1 border-b">
        ...
        <span className="text-end">{RupiahIRD(product.price * product.quantity)}</span>
      </div>
    );
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    const payload = {
      customerName: name,
      tableNumber: !takeaway ? table : null,
      orderItems: cart,
      hasPayed: true,
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <span className={'text-center font-semibold text-lg'}>Bakmi Tepe</span>
        ...
        <ScrollArea className="w-auto flex items-center justify-center mx-5">
          ...
          {cart.length > 0 ? (
            cart.map((item) => <ListCard key={item.menuId} product={item} />)
          ) : (
        ...
        <DialogFooter className="flex-col">
          ...
          <Button variant="success" onClick={handlePlaceOrder} disabled={rawValue < totalPrice}>
