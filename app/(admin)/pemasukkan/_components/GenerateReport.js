...
export const handleExportToExcel = async (filteredOrder, totalRevenue) => {
  ...
  const subtitleRow = worksheet.addRow(['Warung Bakmi Tepe']);
  subtitleRow.font = { name: 'Arial', size: 12, bold: true };
  subtitleRow.alignment = { horizontal: 'center' };
  worksheet.mergeCells(`A${subtitleRow.number}`, `F${subtitleRow.number}`);

  const dateRow = worksheet.addRow([`Dicetak pada: ${moment().format('D MMMM YYYY, HH:mm')}`]);
  ...
  filteredOrder.forEach((item, index) => {
    const menuItems = item.orderItems
      .map((orderItem) => `• ${orderItem.menu?.name || 'N/A'} (x${orderItem.quantity})`)
      .join('\n');

    const rowData = [
      index + 1,
      moment(item.orderTime).format('D MMM YYYY'),
      item.customerName,
      menuItems,
  ...
  worksheet.addRow([]);
  const footerRow = worksheet.addRow(['', '', '', '', 'Total Pemasukkan', totalRevenue]);
  ...
  workbook.xlsx.writeBuffer().then((buffer) => {
    ...
    const fileName = `Laporan-Pemasukan-Bakmitepe-${moment().format('YYYYMMDD-HHmmss')}.xlsx`;
